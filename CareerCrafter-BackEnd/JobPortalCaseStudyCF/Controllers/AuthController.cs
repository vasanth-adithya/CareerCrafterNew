...
namespace JobPortalCaseStudyCF.Controllers
{
    ...
    public class AuthController : ControllerBase
    {
        ...
        [HttpPost]
        [Route("Login")]
        public async Task<ActionResult> Login([FromBody] LoginDTO loginCredentials)
        {
            if (loginCredentials == null) return BadRequest(ModelState); ;
            dynamic user;
            user = await _employerServices.GetEmployerByEmailasync(loginCredentials.Email);

            if (user == null)
            {
                user = await _jobseekerServices.GetJobSeekerByEmailAsync(loginCredentials.Email);
            }
